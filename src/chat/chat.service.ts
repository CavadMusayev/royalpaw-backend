...
const isAudio = dto.type === 'audio';
  ...
  // söhbəti sil (+ bütün mesajları)
async deleteConversation(convId: string) {
    await this.msgRepo.delete({ conversationId: convId });
...
// tək mesaj sil
  async deleteMessage(msgId: string) {
    await this.msgRepo.delete(msgId);
    return { deleted: true };
  }

  // mesaj redaktə (mətn)
  async editMessage(msgId: string, body: string) {
    await this.msgRepo.update(msgId, { body, edited: true });
    return { ok: true };
  }
}